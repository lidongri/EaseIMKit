...
@property (nonatomic,strong) UIImageView *crowmImageView;
...
- (void)_setupViewsWithType:(EMMessageType)aType chatType:(EMChatType)chatType
{
    ...
    _crowmImageView = [[UIImageView alloc] init];
    _crowmImageView.image = [UIImage imageNamed:@"care_vip_kuang"];
    _crowmImageView.userInteractionEnabled = YES;
    _crowmImageView.hidden = YES;
    [self.contentView addSubview:_crowmImageView];
    ...
    if (self.direction == EMMessageDirectionReceive) {
        ...
        [_crowmImageView Ease_makeConstraints:^(EaseConstraintMaker *make) {
            make.top.equalTo(self.contentView).offset(4);
            make.left.equalTo(self.contentView).offset(componentSpacing-3);
        ...
        [_crowmImageView Ease_makeConstraints:^(EaseConstraintMaker *make) {
            make.top.equalTo(self.contentView).offset(4);
            make.right.equalTo(self.contentView).offset(-(componentSpacing-3));
            make.width.equalTo(@(46));
            make.height.equalTo(@(54));
        }];
        ...
        [_nameLabel Ease_makeConstraints:^(EaseConstraintMaker *make) {
            make.top.equalTo(self.avatarView).offset(-3);
            make.right.equalTo(self.avatarView.ease_left).offset(-8);
            make.left.equalTo(self.contentView).offset(componentSpacing);
        }];
        _nameLabel.hidden = YES;
    }
    ...
    if (self.direction == EMMessageDirectionReceive) {
        [_bubbleView Ease_makeConstraints:^(EaseConstraintMaker *make) {
            if (chatType != EMChatTypeChat) {
                make.top.equalTo(self.nameLabel.ease_bottom).offset(10);
            } else {
                make.top.equalTo(self.avatarView.ease_top).offset(0);
            }
            make.top.equalTo(self.nameLabel.ease_bottom).offset(10);
...
- (void)setModel:(EaseMessageModel *)model
{
    ...
    if (model.userDataDelegate && [model.userDataDelegate respondsToSelector:@selector(defaultAvatar)]) {
        if (model.userDataDelegate.defaultAvatar) {
            [_avatarView Ease_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",@"http://indiary.oss-cn-qingdao.aliyuncs.com/media%2Favatar%2Fxwz_20220322R.png"]] completed:nil];
