...
- (void)_updatetextViewHeight
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"BWConsultUpdateOrderTextViewHeightNotify" object:[NSString stringWithFormat:@"%f",height]];
...
- (void)_remakeButtonsViewConstraints
{
    if (self.currentMoreView) {
        ...
        if (self.currentMoreView == self.moreEmoticonView) {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"BWConsultUpdateOrderButtonNotify" object:[NSString stringWithFormat:@"%f",self.currentMoreView.frame.size.height]];
        } else {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"BWConsultUpdateOrderButtonNotify" object:[NSString stringWithFormat:@"%f",self.currentMoreView.frame.size.height+EMVIEWBOTTOMMARGIN]];
        }
    } else {
        [self.bottomLine Ease_remakeConstraints:^(EaseConstraintMaker *make) {
            make.top.equalTo(self.textView.ease_bottom).offset(5);
...
- (void)setQuoteMessage:(EaseMessageModel *)quoteMessage
{
    ...
    if (!quoteMessage) {
        ...
        NSString *nickname;
        if ([quoteMessage.userDataDelegate respondsToSelector:@selector(showName)]) {
            nickname = quoteMessage.userDataDelegate.showName;
