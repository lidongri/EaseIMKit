...
@protocol EaseAnimatedImgViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(EaseAnimatedImgView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface EaseAnimatedImgView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<EaseAnimatedImgViewDebugDelegate> debug_delegate;
#endif

@end


@implementation EaseAnimatedImgView
...
- (void)setAnimatedImage:(EaseAnimatedImg *)animatedImage
{
...
{
    // Presision is set to half of the `kEaseAnimatedImgDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kEaseAnimatedImgDelayTimeIntervalMinimum;
