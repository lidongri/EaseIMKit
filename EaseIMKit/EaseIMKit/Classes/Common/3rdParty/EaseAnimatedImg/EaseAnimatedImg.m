...
const NSTimeInterval kEaseAnimatedImgDelayTimeIntervalMinimum = 0.02;

// An animated image's data size (dimensions * frameCount) category; its value is the max allowed memory (in MB).
// E.g.: A 100x200px GIF with 30 frames is ~2.3MB in our pixel format and would fall into the `EaseAnimatedImgDataSizeCategoryAll` category.
typedef NS_ENUM(NSUInteger, EaseAnimatedImgDataSizeCategory) {
    EaseAnimatedImgDataSizeCategoryAll = 10,       // All frames permanently in memory (be nice to the CPU)
    EaseAnimatedImgDataSizeCategoryDefault = 75,   // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
    EaseAnimatedImgDataSizeCategoryOnDemand = 250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
    EaseAnimatedImgDataSizeCategoryUnsupported     // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, EaseAnimatedImgFrameCacheSize) {
    EaseAnimatedImgFrameCacheSizeNoLimit = 0,                // 0 means no specific limit
    EaseAnimatedImgFrameCacheSizeLowMemory = 1,              // The minimum frame cache size; this will produce frames on-demand.
    EaseAnimatedImgFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one frame ahead will already result in a stutter-free playback.
    EaseAnimatedImgFrameCacheSizeDefault = 5                 // Build up a comfy buffer window to cope with CPU hiccups etc.
};


#if defined(DEBUG) && DEBUG
@protocol EaseAnimatedImgDebugDelegate <NSObject>
@optional
- (void)debug_animatedImage:(EaseAnimatedImg *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(EaseAnimatedImg *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(EaseAnimatedImg *)animatedImage;
@end
#endif


@interface EaseAnimatedImg ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<EaseAnimatedImgDebugDelegate> debug_delegate;
#endif
...
@implementation EaseAnimatedImg
...
- (NSUInteger)frameCacheSizeCurrent
{
    ...
    if (self.frameCacheSizeMax > EaseAnimatedImgFrameCacheSizeNoLimit) {
        frameCacheSizeCurrent = MIN(frameCacheSizeCurrent, self.frameCacheSizeMax);
    }
    
    if (self.frameCacheSizeMaxInternal > EaseAnimatedImgFrameCacheSizeNoLimit) {
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled
{
    ...
    if (self) {
            ...
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        delayTimesForIndexesMutable[@(i)] = [NSNumber numberWithDouble:0.08];
                    } else {
                        skippedFrameCount++;
        ...
        if (optimalFrameCacheSize == 0) {
            ...
            if (animatedImageDataSize <= EaseAnimatedImgDataSizeCategoryAll) {
                _frameCacheSizeOptimal = self.frameCount;
            } else if (animatedImageDataSize <= EaseAnimatedImgDataSizeCategoryDefault) {
                ...
                _frameCacheSizeOptimal = EaseAnimatedImgFrameCacheSizeDefault;
            } else {
                // The predicted size exceeds the limits to build up a cache and we go into low memory mode from the beginning.
                _frameCacheSizeOptimal = EaseAnimatedImgFrameCacheSizeLowMemory;
            }
        } else {
...
{
    EaseAnimatedImg *animatedImage = [[EaseAnimatedImg alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
    ...
    EaseAnimatedImg * __weak weakSelf = self;
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    } else if ([image isKindOfClass:[EaseAnimatedImg class]]) {
        EaseAnimatedImg *animatedImage = (EaseAnimatedImg *)image;
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self.weakProxy selector:@selector(growFrameCacheSizeAfterMemoryWarning:) object:@(EaseAnimatedImgFrameCacheSizeGrowAfterMemoryWarning)];
    ...
    if ((self.memoryWarningCount - 1) <= kGrowAttemptsMax) {
        [self.weakProxy performSelector:@selector(growFrameCacheSizeAfterMemoryWarning:) withObject:@(EaseAnimatedImgFrameCacheSizeGrowAfterMemoryWarning) afterDelay:kGrowDelay];
    }
