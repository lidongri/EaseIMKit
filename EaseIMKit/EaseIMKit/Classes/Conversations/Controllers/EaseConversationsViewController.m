...
@property (nonatomic, strong) NSString *zhuliStr;
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"BW_loadAllConversationsFromDB" object:nil];
//    NSArray *myArr = [[NSUserDefaults standardUserDefaults] objectForKey:@"CUNZAIROOMID"];
//    __weak typeof(self) weakSelf = self;
...
//            }
//
//            if(conv.type == EMConversationTypeGroupChat){
//
////
//                BOOL contains = [myArr containsObject:conv.conversationId];
//
//                if(contains == YES){
//
//
...
//
//
//            if([self.zhuliStr isEqualToString:@"0"]){
//
//                if (conv.type == EMConversationTypeChat) {
//
//                    if([conv.conversationId isEqualToString:@"10112341111"]){
//
//                        continue;
//
//                    }else {
...
//        });
        //获取本地会话
        NSArray *localArray = [EMClient.sharedClient.chatManager getAllConversations];
        if (localArray.count > 0) {
            //说明本地会话已经有了
            [self dealConversationWithArray:localArray mutableArray:totals];
        } else {
            //没有本地会话，去拉服务器的
            [EMClient.sharedClient.chatManager getConversationsFromServer:^(NSArray *aCoversations, EMError *aError) {
                [self dealConversationWithArray:aCoversations mutableArray:totals];
            }];
        }
    });
}

-(void)dealConversationWithArray:(NSArray *)array mutableArray:(NSMutableArray *)totals
{
    __weak typeof(self) weakSelf = self;
    NSArray *conversations = array;
