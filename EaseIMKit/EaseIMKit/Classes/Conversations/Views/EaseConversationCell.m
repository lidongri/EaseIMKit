...
- (void)_addSubViews {
    ...
    _messageBagView = [[UIImageView alloc] initWithFrame:CGRectZero];
    ...
    [self.contentView addSubview:_messageBagView];
}
...
- (void)_setupViewsProperty {
    ...
    _messageBagView.image = [UIImage imageNamed:@"BWMessageBagImage"];
...
- (void)_setupSubViewsConstraints
{
    ...
    [_avatarView Ease_remakeConstraints:^(EaseConstraintMaker *make) {
        ...
        make.left.equalTo(weakSelf.contentView.ease_left).offset(weakSelf.viewModel.avatarEdgeInsets.left + 12);
    ...
    [_detailLabel Ease_remakeConstraints:^(EaseConstraintMaker *make) {
        make.top.equalTo(weakSelf.nameLabel.ease_bottom).offset(weakSelf.viewModel.nameLabelEdgeInsets.bottom + weakSelf.viewModel.detailLabelEdgeInsets.top+2);
        ...
        make.bottom.equalTo(weakSelf.contentView.ease_bottom).offset(weakSelf.viewModel.detailLabelEdgeInsets.bottom - 16);
    }];
    ...
    [_messageBagView Ease_remakeConstraints:^(EaseConstraintMaker *make) {
        make.right.equalTo(weakSelf.contentView.ease_right).offset(-weakSelf.viewModel.timeLabelEdgeInsets.right - 18);
        make.bottom.equalTo(weakSelf.contentView.ease_bottom).offset(-16);
...
//    }
    ...
    EMGroup *groupManager = [EMGroup groupWithId:model.easeId];//获取群组实例
    if (groupManager.isPushNotificationEnabled) {
        self.messageBagView.hidden = YES;
    } else {
        self.messageBagView.hidden = NO;
    }
//    if (model.type == EMConversationTypeGroupChat) {
//        [EMClient.sharedClient.pushManager getSilentModeForConversation:model.easeId conversationType:EMConversationTypeGroupChat completion:^(EMSilentModeResult * _Nullable aResult, EMError * _Nullable aError) {
//            if (aResult.remindType == EMPushRemindTypeAll) {
//                self.messageBagView.hidden = YES;
//            } else {
//                self.messageBagView.hidden = NO;
//            }
